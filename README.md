...
|------|---------|-------------|
| 2026-01-01 | **🤝 Society of Minds (PoC)** | OpenAI-compatible endpoint (`/v1/chat/completions`) working with Open WebUI! Society FSM coordinates 4 LLMs (Claude, Grok, GPT, Gemini) to provide multi-perspective answers. Smart Chairman delegation: consults all LLMs for substantive questions, handles mechanical tasks (tagging, titles) directly. 56% fewer LLM calls, 71% faster through intelligent routing. **Workaround**: Disable "Stream Chat Response" in Open WebUI—streaming support coming in Phase 2. Next: Parallel LLM calls to reduce 60s → ~15s wall-clock time. |
