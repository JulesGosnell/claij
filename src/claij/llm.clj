...
(defn call
  "Call LLM API asynchronously with JSON parsing and retry logic.
   ...
   Args:
     ...
     handler  - Function to call with successful response:
                - Without tools: parsed JSON map
                - With tools returning tool_calls: {:tool_calls [...]}
                - With tools returning content: parsed JSON map
  ...
  [service model prompts handler & [{:keys [registry schema error tools retry-count max-retries]
                                     :or {retry-count 0 max-retries 3}}]]
  (let [registry (or registry svc/default-registry)]
    (log/info (str "      LLM Call: " service "/" model
                   (when tools (str " (with " (count tools) " tools)"))
         ...
         (try
           ...
           (let [parsed-response (json/parse-string (:body response) false)
             ...
             (if (seq tool_calls)
               ...
               ;; Handle content response (parse as JSON)
               (if (nil? content)
