...
(defn removev [p? c] (vec (remove c p?)))
       ...
       (fn [old]
         (-> old
             (dissoc
              ...
              "LoggingMessageNotification"
              "JSONRPCNotification")
             (update-in ["ClientRequest" "anyOf"] removev (partial = {"$ref" "#/definitions/InitializeRequest"})))))))
(comment
...
(comment
  (def n 100)
  (def config {"command" "bash", "args" ["-c", "cd /Users/jules/src/claij && ./bin/mcp-clojure-tools.sh"], "transport" "stdio"})
  (def ic (chan n (map write-str)))
  (def oc (chan n (map read-str)))
  (def stop (start-mcp-bridge config ic oc))

  (>!! ic mcp-initialise-request)
  ...
  (def resource-read-requests
    (map
     ...
     (range)
     (get-in resources-response ["result" "resources"])))
