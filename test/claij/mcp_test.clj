(ns claij.mcp-test
  "Tests for MCP schema generation and cache management.
   ...
   Schema generators now emit JSON Schema format.
   Validation uses claij.schema/validate."
  (:require
   ...
   [claij.mcp.schema :as mcp-schema :refer [tool-response-schema
...
(defn schema-valid?
  ...
  [json-schema value]
  (:valid? (schema/validate json-schema value)))
...
(deftest mcp-test
  ...
  (testing "tool call schema generation"
    (let [tool-cache {"name" "clojure_eval"
      ...
      (is (not (schema-valid? request-schema invalid-request-wrong-name))
          "Wrong tool name should fail")
      (is (not (schema-valid? request-schema invalid-request-missing-arg))
  ...
  (testing "combined tools schema generation (:or)"
    (let [tools-cache [{"name" "add"
      ...
      (is (contains? request-schema "oneOf") "Request schema should be [:or ...]")
      (is (= 2 (count (get request-schema "oneOf"))) ":or should have 2 alternatives")

      ;; Valid requests pass
      (is (schema-valid? request-schema add-request) "Add request should validate")
      (is (schema-valid? request-schema greet-request) "Greet request should validate")

      ;; Invalid request fails (wrong type for 'a')
      (is (not (schema-valid? request-schema invalid-request))
  ...
  (testing "resource read schema generation"
    (let [resources-cache [{"uri" "custom://readme" "name" "README.md"}
      ...
      ;; Valid requests pass
      (is (schema-valid? request-schema valid-request))
      (is (schema-valid? request-schema valid-request-2))
  ...
  (testing "prompt get schema generation"
    (let [prompt-no-args {"name" "system-prompt"
      ...
      (is (= "object" (get schema-no-args "type")) "Prompt schema should be [:map ...]")

      ;; Combined schema uses :or
      (is (contains? combined-schema "oneOf") "Combined schema should be [:or ...]")
      (is (= 2 (count (get combined-schema "oneOf"))) ":or should have 2 alternatives")

      ;; Valid requests pass
      (is (schema-valid? schema-no-args request-no-args))
      (is (schema-valid? schema-required request-with-args))

      ;; Missing required argument fails
      (is (not (schema-valid? schema-required request-missing-required)))))
  ...
  (testing "combined MCP cache schema generation"
    (let [cache {"tools" [{"name" "eval"
      ...
      (is (contains? request-schema "oneOf") "Request schema should be [:or ...]")
      (is (= 4 (count (get request-schema "oneOf"))) "Should have 4 alternatives")

      ;; Empty cache still has logging
      (is (contains? empty-request-schema "oneOf"))
      (is (= 1 (count (get empty-request-schema "oneOf"))) "Empty cache should have only logging")
      ...
      ;; Valid requests pass
      (is (schema-valid? request-schema tool-request))
      (is (schema-valid? request-schema resource-request))
      (is (schema-valid? request-schema prompt-request))
      (is (schema-valid? request-schema logging-request))

      ;; Invalid method fails
      (is (not (schema-valid? request-schema invalid-method)))))
  ...
  (testing "FSM schema functions"
    (let [cache {"tools" [{"name" "clojure_eval"
      ...
      (let [schema (mcp-request-schema-fn context xition)]
        (is (contains? schema "oneOf") "Request schema function should return [:or ...]")
        (is (= 4 (count (get schema "oneOf"))) "Should have 4 alternatives"))

      ;; mcp-response-schema-fn returns [:or ...] schema
      (let [schema (mcp-response-schema-fn context xition)]
        (is (contains? schema "oneOf") "Response schema function should return [:or ...]"))
      ...
      (let [empty-context {"state" {"tools" nil "resources" nil "prompts" nil}}
        ...
        (is (= 1 (count (get schema "oneOf"))) "Empty cache should have only logging"))))
  ...
  (testing "FSM xition schema functions (complete envelope)"
    (let [cache {"tools" [{"name" "clojure_eval"
      ...
      (let [request-schema (mcp-request-xition-schema-fn context llm->servicing)
        ...
        (is (schema-valid? request-schema valid-request) "Valid tool call request should pass")
        (is (schema-valid? response-schema valid-response) "Valid tool response should pass")
        (is (not (schema-valid? request-schema invalid-request)) "Invalid method should fail")))))
...
(deftest json-rpc-envelope-schemas-test
  (testing "Static request envelope validates JSON-RPC 2.0 requests"
    ...
    (is (schema-valid? json-rpc-request-envelope
    ...
    ;; Invalid requests
    (is (not (schema-valid? json-rpc-request-envelope
  ...
  (testing "Static response envelope validates JSON-RPC 2.0 responses"
    ...
    (is (schema-valid? json-rpc-response-envelope
  ...
  (testing "Static notification envelope validates JSON-RPC 2.0 notifications"
    (is (schema-valid? json-rpc-notification-envelope
    ...
    (is (not (schema-valid? json-rpc-notification-envelope
...
(deftest make-envelope-schema-test
  (testing "make-request-envelope-schema builds typed envelopes"
    (let [tools-call-schema (make-request-envelope-schema "tools/call"
      ...
      ;; Wrong method fails
      (is (not (schema-valid? tools-call-schema
  ...
  (testing "make-response-envelope-schema builds typed envelopes"
    (let [tool-result-schema (make-response-envelope-schema
      ...
      (is (not (schema-valid? tool-result-schema
...
(deftest resolve-mcp-tool-input-schema-test
  ...
  (testing "returns {} for unknown tool"
    (is (= {} (resolve-mcp-tool-input-schema test-tool-cache "unknown"))))

  (testing "returns {} for empty cache"
    (is (= {} (resolve-mcp-tool-input-schema {} "any_tool")))))
...
(deftest mcp-tool-schema-fn-integration-test
  (testing "mcp-tool-request-schema-fn works at config time (no cache)"
    ...
    (let [context {} ; No "state" with cache
      ...
      (is (= "object" (get schema "type")) "Should return a map schema")
